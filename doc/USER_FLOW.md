...
graph TD
    ...
    F --> G["🎫 JWT Token 생성 및 반환<br/>access_token, user_id"]
    G --> H["📝 개인정보 입력 페이지"]
    H --> I["📋 입력 항목:<br/>전화번호, 닉네임, 프로필 사진"]
    I --> J["📤 PUT /api/v1/users/user_id"]
    J --> K["❓ 설문조사 페이지"]
    K --> L["📊 설문 항목:<br/>학습 목표, 난이도 선호도, 학습 시간"]
...
graph TD
    ...
    J --> K["🔍 처리 과정:<br/>프레임 캡처, 손 포즈 추출<br/>PyTorch 모델 추론"]
    K --> L["📊 결과 반환<br/>predicted_char, confidence"]
    L --> M["🌐 WebSocket으로 실시간 전송"]
    M --> N["📱 Frontend에서 결과 표시<br/>인식된 문자, 신뢰도 바, 정답/오답"]
    N --> O{모든 문자 완료?}
    O -->|No| Q["▶️ 다음 문자"]
    Q --> I
...
graph TD
    ...
    C --> D["🎯 Word 레슨 목록 페이지"]
    D --> E["예시: 일상용어, 음식, 동물, 감정"]
    E --> F["📌 레슨 선택<br/>예: 일상용어"]
    F --> G["🎓 GET /api/v1/learning/lesson/lesson_id"]
    G --> H["🔤 단어 목록 페이지"]
    H --> I["🎬 각 단어의 영상/이미지<br/>클릭하여 상세 페이지로"]
    I --> J["📝 단어 상세 페이지"]
    J --> K["📺 수어 시범 영상 재생<br/>단어 뜻 설명, 예문"]
    K --> L["🎬 실시간 인식 시작"]
    L --> M["👐 사용자가 수어로 표현<br/>웹캠으로 시연"]
    M --> N["⚙️ recon_word.py"]
    N --> O["🔍 비디오 프레임 수집<br/>MediaPipe 손 추적, PyTorch 모델 추론<br/>신뢰도 계산"]
...
graph TD
    ...
    C --> D["🎯 Sentence 레슨 목록"]
    D --> E["예시: 인사말, 자기소개, 일상 대화"]
    E --> F["📌 문장 선택"]
    F --> G["📝 문장 상세 페이지"]
    G --> H["🎬 문장 뜻, 수어 시범 영상 전체<br/>각 단어별 분해 영상"]
    H --> I["▶️ 시범 영상 재생"]
    I --> J["👐 사용자가 전체 문장을 수어로 표현<br/>웹캠으로 촬영"]
    J --> K["⚙️ recon_video.py"]
    K --> L["🔍 전체 영상 프레임 수집<br/>각 구간별 단어 인식<br/>전체 문장 순서 검증"]
    L --> M["📊 정확도 계산<br/>단어별 정확도 평균"]
    M --> N["📈 결과: accuracy, word_scores"]
    N --> O{정확도 확인}
    O -->|>70% 통과| P["✅ 통과"]
    O -->|≤70% 재시도| Q["❌ 재시도"]
...
graph TD
    ...
    G --> H["🧺 학습 바구니 페이지"]
    H --> I["📋 저장된 모든 항목 표시<br/>재학습 버튼, 삭제 버튼"]
    I --> J{항목 선택}
...
graph TD
    ...
    D --> E3["⚙️ 설정 섹션"]
    D --> E4["🔗 기타 섹션"]
    E1 --> F1["📸 프로필 사진<br/>닉네임, 이메일, 전화번호<br/>프로필 수정, 사진 변경"]
    E2 --> F2["⏱️ 총 학습 시간<br/>📚 완료한 레슨 수<br/>📈 평균 정확도, 최근 학습 날짜<br/>🔥 연속 학습 일수"]
    E3 --> F3["🔔 알림 설정<br/>💬 카톡 알림 활성화<br/>📊 난이도 설정"]
    E4 --> F4["❌ 계정 삭제<br/>🚪 로그아웃"]
    F1 --> G{선택}
    F2 --> G
    F3 --> G
    F4 --> G
    G -->|프로필 수정| H1["✏️ PUT /api/v1/users"]
    G -->|사진 업로드| H2["📸 POST /upload"]
    G -->|알림 설정| H3["🔔 설정 적용"]
    G -->|로그아웃| H4["🚪 로그아웃"]
    H1 --> I["✅ 업데이트 완료"]
    H2 --> I
...
graph TD
    ...
    B --> C["🎯 검색 결과 반환"]
    C --> D["📋 매칭 단어 목록<br/>관련 레슨, 검색 히스토리 자동 저장"]
    D --> E["🌐 검색 결과 페이지"]
...
graph TD
    ...
    A --> C["⏳ 진행 중<br/>progress = user_id<br/>lesson_id, current_word_index"]
...
graph TD
    ...
    H --> J
    I --> J
    J --> K["🔘 사용자 친화적 메시지<br/>재시도 버튼, 홈으로 돌아가기"]
```
...
graph TD
    ...
    B --> E["🏆 모든 레슨 완료"]
    B --> F["📈 특정 정확도 달성"]
    C --> G["🏅 배지 또는 동기 부여 메시지 표시"]
    D --> G
    E --> G
    F --> G
    G --> H{알림 전송?}
    H -->|Yes| I["💬 카톡 또는 이메일 알림 전송"]
```
